            ...
            .statusHistory(
                List.of(
                    JobStatus.builder()
                        ...
                        .build()))
            .build();
    when(jobRepositoryMock.findById(JOB_4_ID)).thenReturn(Optional.of(expectedJob));

    var actual = subject.dispatchTasksBySucceededStatus(JOB_4_ID);

    var actualSucceededJobId = actual.getSucceededJob().getId();
    var actualNotSucceededJobId = actual.getNotSucceededJob().getId();
    var expected =
        new FilteredDetectionJob(
            JOB_4_ID,
            expectedJob.duplicate(actualSucceededJobId),
            expectedJob.duplicate(actualNotSucceededJobId));
    var listArgumentCaptor = ArgumentCaptor.forClass(List.class);
    verify(eventProducerMock, times(1)).accept(listArgumentCaptor.capture());
    var dispatchEvent = ((List<ZDJDispatchRequested>) listArgumentCaptor.getValue()).getFirst();
    assertEquals(expectedJob, dispatchEvent.getJob());
    assertEquals(actualSucceededJobId, dispatchEvent.getSucceededJobId());
    assertEquals(actualNotSucceededJobId, dispatchEvent.getNotSucceededJobId());
