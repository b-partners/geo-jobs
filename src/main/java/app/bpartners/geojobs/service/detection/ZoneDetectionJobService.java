...
public class ZoneDetectionJobService extends JobService<ParcelDetectionTask, ZoneDetectionJob> {
  ...
  public FilteredDetectionJob dispatchTasksBySucceededStatus(String jobId) {
    ...
    eventProducer.accept(
        List.of(
            ZDJDispatchRequested.builder()
                .job(job)
                .succeededJobId(succeededJobId)
                .notSucceededJobId(notSucceededJobId)
                .build()));

    return new FilteredDetectionJob(
        jobId, job.duplicate(succeededJobId), job.duplicate(notSucceededJobId));
  }
