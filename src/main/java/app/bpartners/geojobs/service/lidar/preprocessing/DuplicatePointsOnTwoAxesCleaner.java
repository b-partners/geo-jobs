...
public record DuplicatePointsOnTwoAxesCleaner(Axis axis1, Axis axis2, double axis1Epsilon, double axis2Epsilon, BiFunction<LasPointGeometry, LasPointGeometry, LasPointGeometry> computeNewValue) implements Function<Collection<LasPointGeometry>, Collection<LasPointGeometry>> {
    @Override
    ...
    public static DuplicatePointsOnTwoAxesCleaner xyKeepHighest(double epsilonX, double epsilonY) {
        return new DuplicatePointsOnTwoAxesCleaner(X, Y, epsilonX, epsilonY, (existing, current) -> existing.getCoordinate(Z) > current.getCoordinate(Z) ? existing : current);
    }

    public static DuplicatePointsOnTwoAxesCleaner zyKeepFirst(double epsilonZ, double epsilonY) {
        return new DuplicatePointsOnTwoAxesCleaner(Z, Y, epsilonZ, epsilonY, (existing, ignored) -> existing);
    }
}
